...
"""
Purge unneeded locales on UNIX systems
"""
import glob
import os
import re
...
class Locales:
    ...
    def __init__(self):
        ...
        self.set_locales_to_keep(Options.options.get_languages())
        pass

    def set_locales_to_keep(self, locales_to_keep):
        if not locales_to_keep:
            raise RuntimeError('Found no locales to keep')
        self._purgable_locales = frozenset((locale for locale in Locales.native_locale_names.keys()
                                            if locale not in locales_to_keep))

    def add_xml(self, xml_node):
        self._paths.append(xml_node)

    def localization_paths(self):
        for xml_node in self._paths:
            for (locale, path) in handle_path('', xml_node):
                if locale in self._purgable_locales:
                    yield path
        self._paths = []
...
def handle_path(path, xmldata):
    ...
    for child in xmldata.childNodes:
        for (locale, subpath) in handle_path(path, child):
            yield (locale, subpath)
